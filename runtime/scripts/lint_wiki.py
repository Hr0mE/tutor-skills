...
SKIP_DIRS = {".git", ".venv", ".obsidian", "__pycache__", "raw"}


def load_pages(root_path: Path) -> dict[str, Path]:
    """Everything a wikilink may legitimately point at.

    Obsidian's vault root here is the project root, not wiki/ — so a link is
    either a bare page name, a path relative to wiki/, or a path relative to the
    vault. The last form is how concept pages reach outputs/solutions/, whose
    filenames deliberately duplicate the concept ones."""
    pages: dict[str, Path] = {}
    ...
    for p in sorted(root_path.rglob("*.md")):
        ...
        if SKIP_DIRS & set(rel.parts):
            continue
        pages.setdefault(str(rel.with_suffix("")), p)
        if p.is_relative_to(wiki_dir):
            # Bare names resolve to wiki pages only — otherwise a solutions file
            # would shadow the concept page of the same name.
            pages[p.stem] = p
            pages[str(p.relative_to(wiki_dir).with_suffix(""))] = p
    return pages


def resolve(link: str, pages: dict[str, Path]) -> Path | None:
    """Resolve one wikilink target. Note that a page name may contain dots
    (page titles carry punctuation and dashes), so Path.stem is not usable here."""
    return pages.get(link) or pages.get(link.rsplit("/", 1)[-1])
...
def lint(root: str) -> int:
    ...
    missing_pages = [
        ...
        if count >= 3 and resolve(link, pages) is None
    ]
    if missing_pages:
