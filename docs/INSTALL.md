...
```
/plugin marketplace add Hr0mE/tutor-skills
...
**`/plugin` is not available in this session.** Some environments — a VS Code extension session, or any non-interactive one — do not expose it as a slash command. Call the CLI binary directly instead; the effect is identical:
...
If `claude` is not on your `PATH`, the VS Code extension ships its own copy under `~/.vscode/extensions/anthropic.claude-code-*/resources/native-binary/claude`.

**Newly installed skills do not appear in the current session.** They are picked up at session start. Restart Claude Code before expecting `learning-init` to be invocable.
